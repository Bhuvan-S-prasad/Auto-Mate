...
${truncatedResults}
    ...
    const timeoutId = setTimeout(() => controller.abort(), 30000); // 30 seconds for search synthesis
...
${
  ...
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 10000); // 10 seconds for query generation
